...
void GetDsInfo(
    char            *DsName,            // input .ds full path
    char            *SetName,           // input .ds basename without the .ds
    ...
    // save copies
    Header->DsName = copy_string(DsName);
    Header->SetName = copy_string(SetName);

    // 1st, read resource file
    sprintf(PathName, "%s/%s.res4", Header->DsName, Header->SetName);
    fin = fileopen(PathName, "r");
    ...
    *ChannelOut = Channel;      // return pointer to channel info array
    ...
    sprintf(PathName, "%s/%s.eeg", Header->DsName, Header->SetName);
...
//      Epoch[e].StimulusTime = (float)Itmp / Header->SampleRate;   // 'getSamplePreTrigger()' returns sample number!
    ...
    sprintf(PathName, "%s/ClassFile.cls", Header->DsName);
    ...
    sprintf(PathName, "%s/BadChannels", Header->DsName);
    if((fp = fopen(PathName, "r")) != NULL) {
    ...
    if(BadOut != NULL) {
        ...
        sprintf(PathName, "%s/bad.segments", Header->DsName);
